...
static volatile uint32_t u32LastUpdate = millis();

static volatile uint32_t u32LastRefresh = millis();

static volatile uint8_t u8Refreshs = 0;
...
{
    uint32_t u32Diff = millis() - u32LastUpdate;
    if ((bUpdate) && (u32Diff > 200))
    {
        u8Refreshs = 3;
        bUpdate = false;
    }
    if ((u8Refreshs > 0) || ((bUpdate) && ((millis() - u32LastRefresh) > 1000)))
    {
        FastLED.show();
        delay(1);
        u8Refreshs--;
        u32LastRefresh = millis();
    }
